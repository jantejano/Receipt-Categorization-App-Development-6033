...
const {FiCheck,FiX,FiCrown,FiZap,FiUsers,FiShield,FiExternalLink}=FiIcons;

function PricingCard({plan,isCurrentPlan,isPopular,onSelectPlan,loading}) {
  ...
  const handlePaymentClick=()=> {
    ...
    } else if (!plan.paymentLink && !isCurrentPlan) {
      onSelectPlan(plan);
    }
  };
      ...
      <div className="p-8">
        ...
        <div className="text-center mb-8">
          ...
          <div className="mb-6">
            ...
            {plan.yearlyDiscount && plan.amount > 0 && (
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">
                {plan.limits?.users===-1 ? 'Unlimited' : plan.limits?.users || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Storage</span>
              <span className="font-medium text-gray-900">{plan.limits?.storage || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Monthly Receipts</span>
              <span className="font-medium text-gray-900">
                {plan.limits?.monthlyReceipts===-1 ? 'Unlimited' : plan.limits?.monthlyReceipts || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">API Calls</span>
              <span className="font-medium text-gray-900">
                {plan.limits?.apiCalls===-1 ? 'Unlimited' : plan.limits?.apiCalls===0 ? 'None' : `${plan.limits?.apiCalls}/month`}
              </span>
            </div>
          </div>
        </div>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              {plan.paymentLink && <SafeIcon icon={FiExternalLink} className="w-4 h-4 ml-2" />}
            </>
          ) : plan.amount===0 ? (
            'Switch to Free'
          ) : (
